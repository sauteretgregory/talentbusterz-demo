...
test('candidate-id runtime experiment removes candidate state from the inter-request payload', async () => {
  const registry = createTbzEngineRegistry({ engineMode: 'deterministic' })
  const repository = new SqliteCandidateMemoryRepository({ filename: ':memory:' })
  ...
  try {
    ...
    assert.equal(
      (await repository.get(candidateId)).state_version,
      'v1.4_probe_response_integration'
    )
