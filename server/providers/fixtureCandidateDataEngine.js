...
function assertCanonicalCandidateDataState(artifact) {
  if (!artifact || typeof artifact !== 'object') {
    throw new Error('TBZ fixture CANDIDATE DATA ENGINE: canonical candidate state is required.')
  }
  if (artifact.artifact_type !== 'canonical_candidate_data_state') {
    throw new Error('TBZ fixture CANDIDATE DATA ENGINE: invalid artifact_type.')
  }
  if (artifact.engine_name !== 'TBZ_CANDIDATE_DATA_ENGINE') {
    throw new Error('TBZ fixture CANDIDATE DATA ENGINE: invalid engine_name.')
  }
  if (!artifact.artifact_filename) {
    throw new Error('TBZ fixture CANDIDATE DATA ENGINE: artifact_filename is required.')
  }
  if (!artifact.candidate_data_state?.candidate_id) {
    throw new Error('TBZ fixture CANDIDATE DATA ENGINE: candidate_data_state.candidate_id is required.')
  }
  return artifact
}
...
function getAnswers(input) {
  return Array.isArray(input?.probe_responses)
    ? input.probe_responses.filter(
        (item) => item && typeof item.question_id === 'string' && typeof item.answer === 'string' && item.answer.trim()
      )
    : []
}
...
function updateStudentStatus(candidateState, answer) {
  ...
  candidateState.current_student_status = {
    value: active ? 'active_training_status_declared' : 'not_currently_student_or_in_active_training',
    active,
    evidence_status: 'confirmed',
    evidence_origin_type: 'direct_user_statement',
    data_stability_level: 'volatile',
...
function updateEnglish(candidateState, answer) {
  ...
  candidateState.language_state = Array.isArray(candidateState.language_state)
    ? candidateState.language_state
    : []

  const existing = candidateState.language_state.find(
    (entry) => ['anglais', 'english', 'en'].includes(normalize(entry.language))
  )
...
function updateWorkload(candidateState, answer) {
  ...
  experience.quantified_claims = Array.isArray(experience.quantified_claims)
    ? experience.quantified_claims
    : []
  ...
  if (interviewMatch) {
    experience.quantified_claims.push({
      metric: 'entretiens_hebdomadaires',
      value: Number(interviewMatch[1]),
      evidence_status: 'confirmed',
      evidence_origin_type: 'direct_user_statement',
      value_type: 'probe_response',
      exactness: 'candidate_declared_approximate',
      must_not_be_presented_as_exact_kpi: true
    })
  }

  if (recruitmentMatch) {
    experience.quantified_claims.push({
      metric: 'recrutements_mensuels',
      value: Number(recruitmentMatch[1]),
      evidence_status: 'confirmed',
      evidence_origin_type: 'direct_user_statement',
      value_type: 'probe_response',
      exactness: 'candidate_declared_approximate',
  ...
  experience.probe_workload_response = {
    response: answer,
    evidence_status: 'confirmed',
    evidence_origin_type: 'direct_user_statement'
  }
...
export function reingestProbeResponses(candidateArtifact, probeResponses) {
  ...
  if (!answers.length) {
    throw new Error('TBZ CANDIDATE DATA ENGINE: at least one non-empty probe response is required.')
  }
  ...
  const handlers = {
    MPC_FT_001: updateEsnExperience,
    probe_ft_candidate_education_001: updateEducation,
    probe_ft_candidate_student_status_001: updateStudentStatus,
    MPC_FT_002: updateEnglish,
    MPC_FT_003: updateWorkload
  }
  ...
  const nextVersion = 'v1.4_probe_response_integration'
  ...
  next.artifact_filename = 'candidate_gregory_sauteret_v1.4.json'
  next.materialization_status = 'updated_from_probe_responses'
  next.candidate_data_update_result = {
    engine: 'TBZ_CANDIDATE_DATA_ENGINE',
    engine_version: 'V1',
    candidate_id: candidateState.candidate_id,
    status: 'completed',
    previous_version: previousVersion,
    current_version: nextVersion,
    new_integration_performed: true,
    canonical_state_materialized: true,
    consistency_correction_applied: false,
...
export function createFixtureCandidateDataEngineProvider() {
  return async function fixtureCandidateDataEngine(input = {}) {
    ...
    return responses.length
      ? reingestProbeResponses(canonical, responses)
