...
function getQuestionIds(plan) {
  return [
    ...(plan?.probe_plan?.critical_questions || []),
    ...(plan?.probe_plan?.secondary_questions || [])
  ].map((q) => q?.question_id).filter(Boolean)
}
...
function isInsufficientAnswer(answer) {
  ...
  return n.length < 2 || /^(je ne sais pas|je sais pas|aucune idee|pas d'idee|je ne peux pas|impossible|pas sur|je ne suis pas sur|inconnu|unknown|n\/a|na)$/i.test(n)
}
...
function isContradictoryAnswer(answer) {
  const n = normalizeAnswer(answer)
  return /\b(oui|yes)\b/.test(n) && /\b(non|no)\b/.test(n)
}

function classifyResponseQuality(responses) {
  const usable = responses.filter((r) => !r?.skipped && !isPassAnswer(r?.answer))
  const insufficientQuestionIds = usable.filter((r) => isInsufficientAnswer(r?.answer)).map((r) => r?.question_id).filter(Boolean)
  const contradictoryQuestionIds = usable.filter((r) => isContradictoryAnswer(r?.answer)).map((r) => r?.question_id).filter(Boolean)
  return {
...
function markCycleContinuation(probePlan, originalProbePlan, cycleNumber, candidateDataState, currentScore) {
  ...
  const potentialScoreGain = cycleComplete
    ? Math.max(10, estimatePotentialScoreGain({ remainingQuestionCount, currentScore }))
    : 0
  ...
  next.probe_result = {
    ...
    enrichment_cycle: {
      ...
      answered_question_count: answeredThisCycle,
      estimated_potential_score_gain: potentialScoreGain,
      estimated_gain_basis: cycleComplete
        ? 'continued_enrichment_is_estimated_to_be_capable_of_adding_up_to_ten_match_points'
        : 'cycle_in_progress',
...
function applyAdaptiveDecision(probePlan, responseQuality, originalProbePlan) {
  ...
  next.loop_closure = {
    ...
    remaining_question_ids: getQuestionIds(next)
  }
  next.probe_result = {
    ...(next.probe_result || {}),
...
export async function processProbeResponses({
  ...
  if (control === ENRICHMENT_DECISIONS.STOP) {
    ...
    finalPlan.loop_closure = {
      ...(finalPlan.loop_closure || {}),
      status: 'complete',
      decision: ENRICHMENT_DECISIONS.STOP,
      decision_reason: 'candidate_declined_further_profile_enrichment'
    }
    finalPlan.probe_result = {
      ...(finalPlan.probe_result || {}),
      loop_status: 'complete',
      adaptive_decision: ENRICHMENT_DECISIONS.STOP
    }
  ...
  if (control === ENRICHMENT_DECISIONS.CONTINUE && actualResponses.length === 0) {
    ...
    const nextPlan = createOfferIndependentEnrichmentPlan(nextCycleNumber)
    ...
    return {
      ...
      current_score: previousScore,
      score_delta: 0,
      probe_cycle_status: 'open',
      probe_adaptive_decision: ENRICHMENT_DECISIONS.CONTINUE,
  ...
  if (!actualResponses.some((item) => item?.question_id && (item?.answer?.trim() || item?.skipped))) {
  ...
  const candidateExecution = await executeEngine(engineRegistry, ENGINE_IDS.CANDIDATE, {
    candidate_data_state: candidateDataState,
    probe_responses: actualResponses,
  ...
  if (responseQuality.status === 'usable') {
    canonicalProbePlan = markCycleContinuation(canonicalProbePlan, probePlan, cycleNumber, updatedCandidate, currentScore)
  }
  ...
  return {
    ...
    probe_cycle_status: finalState.status,
    probe_adaptive_decision: finalState.decision,
