...
function getQuestionIds(probePlan) { return [...(probePlan?.probe_plan?.critical_questions || []), ...(probePlan?.probe_plan?.secondary_questions || [])].map((question) => question?.question_id).filter(Boolean) }
function normalizeAnswer(answer) { return String(answer || '').normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase().trim() }
function isInsufficientAnswer(answer) { const normalized = normalizeAnswer(answer); return normalized.length < 2 || /^(je ne sais pas|je sais pas|aucune idee|pas d'idee|je ne peux pas|impossible|pas sur|je ne suis pas sur|inconnu|unknown|n\/a|na)$/i.test(normalized) }
function isPassAnswer(answer) { return /^(passer|passe|skip|je passe|sans reponse)$/i.test(normalizeAnswer(answer)) }
function isContradictoryAnswer(answer) { const normalized = normalizeAnswer(answer); return /\b(oui|yes)\b/.test(normalized) && /\b(non|no)\b/.test(normalized) }
function classifyResponseQuality(responses) { const usable = responses.filter((response) => !response?.skipped && !isPassAnswer(response?.answer)); const insufficientQuestionIds = usable.filter((response) => isInsufficientAnswer(response?.answer)).map((response) => response?.question_id).filter(Boolean); const contradictoryQuestionIds = usable.filter((response) => isContradictoryAnswer(response?.answer)).map((response) => response?.question_id).filter(Boolean); return { status: contradictoryQuestionIds.length ? 'contradictory' : insufficientQuestionIds.length ? 'insufficient' : 'usable', insufficient_question_ids: insufficientQuestionIds, contradictory_question_ids: contradictoryQuestionIds } }
...
function markAwaitingContinuation(probePlan, cycleNumber, candidateDataState, currentScore) {
  ...
  next.loop_closure = { ...(next.loop_closure || {}), status, decision, decision_reason: canContinue ? 'five_question_enrichment_cycle_completed_waiting_for_candidate_choice' : 'no_remaining_candidate_answerable_gap', remaining_question_ids: batch.map((question) => question.question_id) }
...
function applyAdaptiveDecision(probePlan, responseQuality) { if (responseQuality.status === 'usable') return probePlan; const next = structuredClone(probePlan); next.loop_closure = { ...(next.loop_closure || {}), status: 'needs_clarification', decision: 'clarification_required', decision_reason: responseQuality.status === 'contradictory' ? 'one_or_more_candidate_answers_contain_conflicting_assertions' : 'one_or_more_candidate_answers_are_insufficient_to_stabilize_evidence', insufficient_question_ids: responseQuality.insufficient_question_ids, contradictory_question_ids: responseQuality.contradictory_question_ids }; next.probe_result = { ...(next.probe_result || {}), loop_status: 'needs_clarification', adaptive_decision: 'clarification_required', decision_reason: next.loop_closure.decision_reason }; return next }
...
export async function processProbeResponses({ engineRegistry, candidateDataState, jobDataState, probePlan, responses = [], previousMatchState = null, continueEnrichment = null, cycleNumber = 1 }) {
  const control = responses.find((response) => response?.control === ENRICHMENT_DECISIONS.CONTINUE || response?.control === ENRICHMENT_DECISIONS.STOP)?.control || continueEnrichment
  const actualResponses = responses.filter((response) => !response?.control)
