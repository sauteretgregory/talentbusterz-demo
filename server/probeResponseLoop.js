...
function getQuestionIds(probePlan) {
  return [...(probePlan?.probe_plan?.critical_questions || []), ...(probePlan?.probe_plan?.secondary_questions || [])].map((question) => question?.question_id).filter(Boolean)
}
...
function classifyResponseQuality(responses) {
  ...
  return { status: contradictoryQuestionIds.length ? 'contradictory' : insufficientQuestionIds.length ? 'insufficient' : 'usable', insufficient_question_ids: insufficientQuestionIds, contradictory_question_ids: contradictoryQuestionIds }
}
...
function markAwaitingContinuation(probePlan, cycleNumber, candidateDataState, currentScore) {
  ...
  const currentCycleQuestionCount = getQuestionIds(probePlan).length
...
function applyAdaptiveDecision(probePlan, responseQuality) {
  ...
  next.probe_result = { ...(next.probe_result || {}), loop_status: 'needs_clarification', adaptive_decision: 'clarification_required', decision_reason: next.loop_closure.decision_reason }
  return next
}
