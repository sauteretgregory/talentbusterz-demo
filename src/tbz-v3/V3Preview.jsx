...
export default function V3Preview() {
  ...
  async function submitAnswer(event) {
    ...
    try {
      const result = await callProbe({
        ...
        responses: [{ question_id: question.question_id, ...(skipped ? { skipped: true } : { answer: answer.trim() }) }],
      ...
      setPreviousScore(oldScore); setScoreDelta(typeof result.score_delta === 'number' ? result.score_delta : null); setAnswer(''); setSkipped(false)
      const nextQuestions = questionsOf(result.canonical_probe_plan)
      if (nextQuestions.length > 0) {
        setQuestionIndex(0)
        setStatus(`MATCH recalculé après cette réponse. Le profil candidat est enrichi. Question suivante du cycle ${cycleNumber}.`)
      } else {
        setQuestionIndex(0)
        setStatus('Les 5 questions du cycle ont été traitées. Vous pouvez maintenant choisir de continuer ou de passer au CV.')
      }
    } catch (err) { setError(err.message); setStatus('') }
  }
  ...
  async function chooseContinuation(decision) {
    ...
    try {
      ...
      setAnalysis((current) => ({ ...current, candidate: result.canonical_candidate_data_state, match: result.canonical_match_state, probePlan: result.canonical_probe_plan }))
      if (decision === 'continue_enrichment') { setCycleNumber((value) => value + 1); setQuestionIndex(0); setAnswer(''); setSkipped(false); setStatus('Nouveau cycle prêt : jusqu’à 5 questions. Chaque réponse repassera immédiatement par le MATCH.') }
      else setStatus('Enrichissement arrêté. Handoff : CV ENGINE → RENDER ENGINE.')
    } catch (err) { setError(err.message); setStatus('') }
  }
  ...
  return (
    <main style={{ maxWidth: 780, margin: '0 auto', padding: '44px 24px 80px', fontFamily: 'Arial, sans-serif' }}>
      ...
      {analysis && <>
        ...
        {currentQuestion && probeStatus !== 'needs_clarification' && <form onSubmit={submitAnswer} style={{ marginTop: 24 }}>
          <section style={styles.card}>
            <h2 style={{ marginTop: 0 }}>Cycle {cycleNumber} — question {questionIndex + 1}/5</h2>
            <p style={{ opacity: .7 }}>Une seule réponse à la fois : chaque réponse enrichit le profil puis déclenche immédiatement un nouveau MATCH.</p>
            <div style={{ marginTop: 18, padding: 20, border: '1px solid #d9e1ee', borderRadius: 14 }}>
              <label style={{ display: 'block', fontWeight: 800, marginBottom: 12 }}>{currentQuestion.question}</label>
              <textarea rows="5" disabled={skipped} value={answer} onChange={(event) => setAnswer(event.target.value)} placeholder="Votre réponse…" style={{ width: '100%', boxSizing: 'border-box', padding: 12, borderRadius: 8, border: '1px solid #cbd5e1' }} />
              <div style={{ display: 'flex', gap: 10, marginTop: 12 }}>
                <button type="button" onClick={() => setSkipped((value) => !value)} style={{ ...styles.button, background: '#f1f5f9' }}>{skipped ? 'Question passée — cliquer pour répondre' : 'Passer cette question'}</button>
                <button type="submit" style={{ ...styles.button, ...styles.primary, flex: 1 }}>{skipped ? 'Passer et recalculer le MATCH' : 'Répondre et recalculer le MATCH'}</button>
              </div>
            </div>
          </section>
        </form>}

        {probeStatus === 'needs_clarification' && <section style={{ ...styles.card, marginTop: 24 }}><h2 style={{ marginTop: 0 }}>Clarification nécessaire</h2><p>Une réponse insuffisante ou contradictoire doit être corrigée avant de poursuivre.</p></section>}

        {cycleFinished && <section style={{ ...styles.card, marginTop: 24 }}>
          <h2 style={{ marginTop: 0 }}>Cycle {cycleNumber} terminé</h2>
          <p>Les réponses de ce cycle sont maintenant intégrées au <strong>profil candidat persistant</strong>. Le MATCH a été recalculé après chaque réponse.</p>
          <p><strong>Oui</strong> : nouveau cycle de 5 questions. <strong>Non</strong> : stop PROBE → CV → Render.</p>
