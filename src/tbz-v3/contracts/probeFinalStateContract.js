export const PROBE_FINAL_STATE_CONTRACT_VERSION = 'v1.1'

export const PROBE_FINAL_STATE_STATUSES = ['open', 'needs_clarification', 'awaiting_continuation', 'complete']

export const PROBE_FINAL_STATE_DECISIONS = ['continue_probe', 'clarification_required', 'continue_enrichment', 'stop_enrichment', 'complete']
...
export function createProbeFinalState(probePlan) {
  if (!probePlan || typeof probePlan !== 'object') throw new Error('TBZ: PROBE final state requires a canonical probe plan.')
  if (probePlan.artifact_type !== 'canonical_probe_plan') throw new Error('TBZ: PROBE final state requires canonical_probe_plan.')
  ...
  if (!PROBE_FINAL_STATE_STATUSES.includes(status)) throw new Error('TBZ: PROBE final state has an unsupported status.')
  if (!PROBE_FINAL_STATE_DECISIONS.includes(decision)) throw new Error('TBZ: PROBE final state has an unsupported decision.')
